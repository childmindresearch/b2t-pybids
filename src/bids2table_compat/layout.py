...
class BIDSLayout:
    ...
    def __init__(
        ...
        **kwargs
        ...
        # Validate parameter (b2t always validates, so we just warn)
        if not validate:
            # This is fine - most users disable validation for speed
            pass
        else:
            warnings.warn(
                "validate=True: bids2table always validates against BIDS schema",
                UserWarning,
                stacklevel=2
            )
    ...
    def _add_derivatives(self, derivatives: Union[str, Path, List[Union[str, Path]]]):
        ...
        for deriv_path in derivatives:
            ...
            deriv_tab = b2t.index_dataset(str(deriv_path))
